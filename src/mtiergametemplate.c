 ...
 * the in-game integer representation of the position, and initialize the
 * hash window to the appropriate tier.
 * 
 ...
 * position hash within the tier. Otherwise, return NULL_POSITION.
 * 
...
POSITION StringToPosition(char *positionString) {
	...
	if (ParseStandardOnelinePositionString(positionString, &turn, &board)) {
        // Set `tier` to the tier that this position indicated by the positionString 
        // belongs to, then initialize the hash window to that tier.
        TIER tier = 0;
